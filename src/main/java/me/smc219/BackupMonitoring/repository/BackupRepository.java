...
public class BackupRepository {
	...
	public List<Backup> findBackupByName(String name) {
		return em.createQuery("select b from Backup b where b.name like :name", Backup.class).setParameter("name", name).getResultList();
	}

	public Backup findBackupById(Long id) {
		return em.find(Backup.class, id);
	}

	public List<Backup> findBackupByTime(LocalDateTime startTime, LocalDateTime endTime) {
